...
class MyOrderController extends GetxController{
  ...
  getTotalPrice() {
    ...
    for (var i = 0; i < orders.length; i++){
      sum += orders[i]['items_price'] * orders[i]['count'] - orders[i]['items_discount'];
      if(i == orders.length-1){
        // return Future.delayed(const Duration(seconds: 1), () => sum);
        return sum;
      }
    }
    update();
  ...
  formatCart() {
    ...
    for (int i = myCartController.data.length-1; i >= 0 ; i--){

      if(myCartController.data[i].cartUsersid == myServices.sharedPreferences.getInt("id")!){
        myCartController.deleteFromCart(myCartController.data[i].cartId!);
      }
    }
